...
describe("Record", () => {
  it("You can build a record from entries", () => {
  ...
  it("You can get an array of tuples from a record", () => {
    const record = {
      b: 2,
      a: 3,
    };
    ...
    const result = pipe(record, R.toArray);
  ...
  it("You can map values of record's entries", () => {
    ...
    const result = pipe(record, R.map(isEven));
  ...
  it("You can filter records, keeping only entries with a value that satisfies a predicate", () => {
    const isEven = (x: number) => x % 2 === 0;
    ...
    const result = pipe(record, R.filter(isEven));

    // ⬆⬆⬆⬆ Code here ⬆⬆⬆⬆

    expect(result).toEqual(expect.objectContaining({ b: 2 }));
  });
  it("You can filter records of Options, keeping only the Some values", () => {
    const record = {
      a: O.some(3),
      b: O.none,
    };
    ...
    const result = pipe(record, R.filterMap(identity));

    // ⬆⬆⬆⬆ Code here ⬆⬆⬆⬆

    expect(result).toEqual(expect.objectContaining({ a: 3 }));
  });
  it("You can filter records with values satisfying a predicate and map the retained values", () => {
    ...
    const result = pipe(record, R.filterMap(mapIfEven));
  ...
  it("You can take a Record of functions and a value to produce a Record with the processed input", () => {
